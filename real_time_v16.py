#!/usr/bin/env python3
# 乒乓球速度追蹤系統 v16 (MODIFIED FOR FAST BALLS & SPEED CALIBRATION)
...
# MODIFICATION: Increased closing kernel size to connect fragmented traces of very fast balls
CLOSING_KERNEL_SIZE_FMO = (35, 35) # Was (25, 25)
...
# MODIFICATION: Added a calibration factor to correct systematic overestimation of speed. (1.0 / 1.3 ≈ 0.77)
SPEED_CALIBRATION_FACTOR = 0.77
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM,
                 ...
                 speed_calibration_factor=SPEED_CALIBRATION_FACTOR): # MODIFICATION: Add parameter
        ...
        # MODIFICATION: Add speed calibration factor
    ...
    def _select_best_ball_candidate(self, candidates):
        ...
        for ball_info in candidates:
            ...
            # MODIFICATION: Relaxed distance check from 0.4 to 0.7 to allow tracking of very fast balls
            if distance > self.frame_width * 0.7: 
    ...
    def _calculate_ball_speed(self):
        ...
        delta_t = t2 - t1
        if delta_t > 0.0001:
            speed_cm_per_second = dist_cm / delta_t
            # MODIFICATION: Apply speed calibration factor here
            speed_kmh = (speed_cm_per_second * KMH_CONVERSION_FACTOR) * self.speed_calibration_factor
    ...
    def run(self):
        print("=== Ping Pong Speed Tracker (v16) ===")
        ...
        print(f"Speed Calibration Factor: {self.speed_calibration_factor:.3f}") # MODIFICATION: Print factor
        if self.debug_mode: print("Debug mode ENABLED.")

        self.running = True
        self.reader.start()
        
        window_name = 'Ping Pong Speed Tracker v16'
...
def main():
    parser = argparse.ArgumentParser(description='Ping Pong Speed Tracker v16')
    ...
    # MODIFICATION: Add command-line argument for speed calibration factor
    parser.add_argument('--speed_factor', type=float, default=SPEED_CALIBRATION_FACTOR, help='Speed calibration factor to correct systematic errors.')
    ...
    tracker = PingPongSpeedTracker(
        ...
        speed_calibration_factor=args.speed_factor # MODIFICATION: Pass factor to constructor
    )
    tracker.run()
