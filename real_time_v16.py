#!/usr/bin/env python3
# 乒乓球速度追蹤系統 v17 (WITH PERSISTENT AREA CALIBRATION)
# Calibrates once, saves data to a file, and auto-loads on next run.
...
import json # NEW: For saving/loading calibration data
...
CALIBRATION_FILENAME = "calibration_data.json" # NEW: File to store calibration data
...
# FrameData, EventRecord, FrameReader 類別與 v16 完全相同，此處為簡潔省略
# (請從 v16 複製貼上這三個類別的完整程式碼)
class FrameData:
    def __init__(self, frame=None, roi_sub_frame=None, ball_position_in_roi=None,
        ...
        self.frame = frame; self.roi_sub_frame = roi_sub_frame; self.ball_position_in_roi = ball_position_in_roi
        self.ball_contour_in_roi = ball_contour_in_roi; self.current_ball_speed_kmh = current_ball_speed_kmh
        self.display_fps = display_fps; self.is_counting_active = is_counting_active
        ...
        self.debug_display_text = debug_display_text; self.frame_counter = frame_counter
        self.trajectory_points_global = []; self.viz_text = viz_text

class EventRecord:
    def __init__(self, ball_x_global, timestamp, speed_kmh, predicted=False):
        self.ball_x_global = ball_x_global; self.timestamp = timestamp; self.speed_kmh = speed_kmh
        self.predicted = predicted; self.processed = False

class FrameReader:
    def __init__(self, video_source, target_fps, use_video_file, frame_width, frame_height):
        self.video_source = video_source; self.target_fps = target_fps; self.use_video_file = use_video_file
    ...
    def _configure_capture(self, frame_width, frame_height):
        ...
        if not self.cap.isOpened(): raise IOError(f"Cannot open video source: {self.video_source}")
    def _read_frames(self):
        while self.running:
            if not self.frame_queue.full():
                ret, frame = self.cap.read()
                if not ret: self.running = False; self.frame_queue.put((False, None)); break
                self.frame_queue.put((True, frame))
            else: time.sleep(1.0 / (self.target_fps * 2))
    def start(self): self.running = True; self.thread.start()
    def read(self):
    ...
    def get_properties(self): return self.actual_fps, self.frame_width, self.frame_height
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM,
        ...
        # ... (初始化參數與 v16 相同) ...
        self.debug_mode = debug_mode; self.use_video_file = use_video_file; self.video_file_path = video_file_path
        self.target_fps = target_fps; self.reader = FrameReader(video_source, target_fps, use_video_file, frame_width, frame_height)
        self.actual_fps, self.frame_width, self.frame_height = self.reader.get_properties()
        self.display_fps = self.actual_fps; self.table_length_cm = table_length_cm; self.detection_timeout_s = detection_timeout_s
        self.roi_start_x = int(self.frame_width * DEFAULT_ROI_START_RATIO); self.roi_end_x = int(self.frame_width * DEFAULT_ROI_END_RATIO)
        self.roi_top_y = 0; self.roi_bottom_y = int(self.frame_height * DEFAULT_ROI_BOTTOM_RATIO); self.roi_width_px = self.roi_end_x - self.roi_start_x
        self.trajectory = deque(maxlen=MAX_TRAJECTORY_POINTS); self.current_ball_speed_kmh = 0; self.last_detection_timestamp = time.time()
        self.area_history = deque(maxlen=AREA_SMOOTHING_WINDOW)
        self.calibration_mode = 'none'
        self.calibrated_area_near_px = None; self.calibrated_area_far_px = None
        ...
        self.frame_counter = 0; self.last_frame_timestamp_for_fps = time.time(); self.frame_timestamps_for_fps = deque(maxlen=MAX_FRAME_TIMES_FPS_CALC)
        self.center_x_global = self.frame_width // 2; self.net_crossing_direction = net_crossing_direction
        self.max_net_speeds_to_collect = max_net_speeds; self.collected_net_speeds = []; self.collected_relative_times = []
        self.last_recorded_net_speed_kmh = 0; self.last_ball_x_global = None; self.output_generated_for_session = False
        self.is_counting_active = False; self.count_session_id = 0; self.timing_started_for_session = False
        self.first_ball_crossing_timestamp = None; self.event_buffer_center_cross = deque(maxlen=EVENT_BUFFER_SIZE_CENTER_CROSS)
        self.running = False; self.file_writer_executor = concurrent.futures.ThreadPoolExecutor(max_workers=2)
        self.ball_on_left_of_center = False; self.last_committed_crossing_time = 0
        self.EFFECTIVE_CROSSING_COOLDOWN_S = 0.2; self.CENTER_ZONE_WIDTH_PIXELS = self.frame_width * 0.05

        self._precalculate_overlay()
        self._load_calibration() # NEW: Attempt to load calibration data on startup
        ...
        try:
            if os.path.exists(CALIBRATION_FILENAME):
                with open(CALIBRATION_FILENAME, 'r') as f:
                    data = json.load(f)
                    self.calibrated_area_near_px = data.get('calibrated_area_near_px')
                    self.calibrated_area_far_px = data.get('calibrated_area_far_px')
                    if self.calibrated_area_near_px and self.calibrated_area_far_px:
                        print(f"--- Calibration data loaded successfully from '{CALIBRATION_FILENAME}' ---")
                        print(f"Near Area: {self.calibrated_area_near_px:.2f}, Far Area: {self.calibrated_area_far_px:.2f}")
                    else:
                        print(f"--- '{CALIBRATION_FILENAME}' is incomplete. Please recalibrate. ---")
                        self.calibrated_area_near_px = None
                        self.calibrated_area_far_px = None
            else:
                print(f"--- No calibration file found. Please perform first-time calibration. ---")
        except (IOError, json.JSONDecodeError) as e:
            print(f"Error loading calibration file: {e}. Please recalibrate.")
            self.calibrated_area_near_px = None
            self.calibrated_area_far_px = None

    # NEW: Function to save calibration data
    def _save_calibration(self):
        if self.calibrated_area_near_px is not None and self.calibrated_area_far_px is not None:
            data = {
                'calibrated_area_near_px': self.calibrated_area_near_px,
                'calibrated_area_far_px': self.calibrated_area_far_px,
                'timestamp': datetime.datetime.now().isoformat()
            }
            try:
                with open(CALIBRATION_FILENAME, 'w') as f:
                    json.dump(data, f, indent=4)
                print(f"--- Calibration data saved to '{CALIBRATION_FILENAME}' ---")
            except IOError as e:
                print(f"!!! Error saving calibration file: {e} !!!")
        else:
            print("--- Calibration data is incomplete. Not saving. ---")
    ...
    def _precalculate_overlay(self):
        ...
        self.instruction_text = "SPACE: Count | C: Re-Calibrate | D: Debug | Q/ESC: Quit"

    # handle_calibration_keypress 函式修改為在完成後自動儲存
    def handle_calibration_keypress(self, key):
        if key == ord('c'):
            # ... (啟動校準的邏輯不變) ...
            if self.calibration_mode == 'none':
                self.calibration_mode = 'near'
                print("--- CALIBRATION START: Place ball at NEAR side, then press 'S' to set. ---")
            elif self.calibration_mode == 'near':
                self.calibration_mode = 'far'
                print("--- CALIBRATION: Place ball at FAR side, then press 'S' to set. ---")
            ...
            if not self.area_history:
                print("!!! Calibration Error: No ball detected. Cannot set area. !!!")
                return
            ...
            if self.calibration_mode == 'near':
                ...
                self.calibration_mode = 'far'
                print("--- CALIBRATION: Now place ball at FAR side, then press 'S' to set. ---")
            elif self.calibration_mode == 'far':
                ...
                if self.calibrated_area_near_px is not None and self.calibrated_area_near_px <= self.calibrated_area_far_px:
                    print("!!! WARNING: Near area is <= Far area. Calibration might be incorrect. !!!")
                
                self.calibration_mode = 'none' # End calibration
                self._save_calibration() # NEW: Automatically save data upon completion
    
    # process_single_frame 函式開頭加入修正尺寸不符的程式碼
    def process_single_frame(self, frame):
        # FIX: Ensure incoming frame matches system dimensions
        h, w, _ = frame.shape
        if h != self.frame_height or w != self.frame_width:
            frame = cv2.resize(frame, (self.frame_width, self.frame_height))

        # ... (後續的 process_single_frame 邏輯與 v16 相同) ...
        ...
        if motion_mask_roi is not None:
            ...
            if ball_pos_in_roi:
                self.area_history.append(ball_area)
                if self.calibrated_area_near_px is not None: self._calculate_ball_speed() # Only calculate if calibrated
        ...
        else: viz_text = "Calibrated. Ready to track." # NEW: Confirmation message
        debug_text = None
        if self.debug_mode:
            smoothed_area = sum(self.area_history) / len(self.area_history) if self.area_history else 0
            debug_text = f"SmArea:{smoothed_area:.1f} N_A:{self.calibrated_area_near_px:.1f} F_A:{self.calibrated_area_far_px:.1f}"
        frame_data = FrameData(frame=frame, roi_sub_frame=roi_sub_frame, ball_position_in_roi=ball_pos_in_roi,
                               ...
                               collected_net_speeds=list(self.collected_net_speeds), last_recorded_net_speed_kmh=self.last_recorded_net_speed_kmh,
                               collected_relative_times=list(self.collected_relative_times), debug_display_text=debug_text,
                               frame_counter=self.frame_counter, viz_text=viz_text)
        if self.trajectory: frame_data.trajectory_points_global = [(int(p[0]), int(p[1])) for p in self.trajectory]
        return frame_data

    def run(self):
        print("=== Ping Pong Speed Tracker (v17 - Persistent Calibration) ===")
        print(self.instruction_text)
        # NEW: Message changes based on loaded calibration data
        if self.calibrated_area_near_px is None:
            print(f"!!! IMPORTANT: System requires calibration. Press 'C' to begin. !!!")
        else:
            print("--- System is calibrated and ready. Press SPACE to start counting. ---")
        if self.debug_mode: print("Debug mode ENABLED.")

        self.running = True; self.reader.start()
        window_name = 'Ping Pong Speed Tracker v17 (Persistent Cal)'
    ...
    # 其餘所有函式 (_get_ratio_from_smoothed_area, _update_display_fps, _preprocess_frame,
    # _detect_fmo, _detect_ball_in_roi, _select_best_ball_candidate, toggle_counting,
    # _record_potential_crossing, _process_crossing_events, _calculate_ball_speed,
    # _calculate_real_distance_cm_area_based, _generate_outputs_async,
    # _create_output_files, _draw_visualizations, _check_timeout_and_reset)
    # 保持與 v16 相同即可，此處為簡潔省略。
    # 請從 v16 複製貼上這些函式的完整程式碼。
    # 提醒：我已將 process_single_frame 的修改納入上方，您只需複製貼上其他未變動的函式。
    def _get_ratio_from_smoothed_area(self, smoothed_area):
        ...
        sqrt_area = math.sqrt(max(1, smoothed_area)); sqrt_area_near = math.sqrt(self.calibrated_area_near_px); sqrt_area_far = math.sqrt(self.calibrated_area_far_px)
    ...
    def _update_display_fps(self):
        if self.use_video_file: self.display_fps = self.actual_fps; return
        now = time.monotonic(); self.frame_timestamps_for_fps.append(now)
    ...
    def _detect_fmo(self):
        ...
        diff1 = cv2.absdiff(f1, f2); diff2 = cv2.absdiff(f2, f3); motion_mask = cv2.bitwise_and(diff1, diff2)
        try: _, thresh_mask = cv2.threshold(motion_mask, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        except cv2.error: _, thresh_mask = cv2.threshold(motion_mask, THRESHOLD_VALUE_FMO, 255, cv2.THRESH_BINARY)
        if OPENING_KERNEL_SIZE_FMO[0] > 0: opened_mask = cv2.morphologyEx(thresh_mask, cv2.MORPH_OPEN, self.opening_kernel)
    ...
    def _detect_ball_in_roi(self, motion_mask_roi):
        ...
        for i in range(1, num_labels):
            ...
            if MIN_BALL_AREA_PX < area < MAX_BALL_AREA_PX:
                w_roi, h_roi = stats[i, cv2.CC_STAT_WIDTH], stats[i, cv2.CC_STAT_HEIGHT]; cx_roi, cy_roi = centroids[i]
                circularity = 0; contour_to_store = None
                if max(w_roi, h_roi) > 0:
                    component_mask = (labels == i).astype(np.uint8) * 255
                    contours, _ = cv2.findContours(component_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    if contours: cnt = contours[0]; contour_to_store = cnt; perimeter = cv2.arcLength(cnt, True)
                    if perimeter > 0: circularity = 4 * math.pi * area / (perimeter * perimeter)
                potential_balls.append({'position_roi': (int(cx_roi), int(cy_roi)), 'area': area, 'circularity': circularity, 'contour_roi': contour_to_store})
        ...
        cx_roi, cy_roi = best_ball_info['position_roi']; cx_global = cx_roi + self.roi_start_x; cy_global = cy_roi + self.roi_top_y
        current_timestamp = time.monotonic();
        if self.use_video_file: current_timestamp = self.frame_counter / self.actual_fps
        self.last_detection_timestamp = time.monotonic(); ball_area = best_ball_info['area']
    ...
    def _select_best_ball_candidate(self, candidates):
        ...
        for ball_info in candidates:
            cx_roi, cy_roi = ball_info['position_roi']; cx_global, cy_global = cx_roi + self.roi_start_x, cy_roi + self.roi_top_y
            distance = math.hypot(cx_global - last_x_global, cy_global - last_y_global); ball_info['distance_from_last'] = distance
        ...
        for ball_info in candidates:
            ball_info['score'] = (0.3 / (1.0 + ball_info['distance_from_last'])) + (0.5 * ball_info['consistency']) + (0.2 * ball_info['circularity'])
        return max(candidates, key=lambda b: b['score'])
    def toggle_counting(self):
        if self.calibration_mode != 'none': print("Please exit calibration mode first."); return
        if self.calibrated_area_near_px is None: print("System is not calibrated. Press 'C' to calibrate first."); return
        ...
        if self.is_counting_active:
            ...
            self.timing_started_for_session = False; self.first_ball_crossing_timestamp = None; self.event_buffer_center_cross.clear()
            self.output_generated_for_session = False; self.ball_on_left_of_center = False; self.last_committed_crossing_time = 0; self.last_ball_x_global = None
    ...
    def _record_potential_crossing(self, ball_x_global, ball_y_global, current_timestamp):
        ...
        if self.last_ball_x_global is not None and self.last_ball_x_global >= self.center_x_global and ball_x_global < self.center_x_global and not self.ball_on_left_of_center: crossed_r_to_l_strictly = True
        if crossed_r_to_l_strictly and self.current_ball_speed_kmh > 0.1: self.event_buffer_center_cross.append(EventRecord(ball_x_global, current_timestamp, self.current_ball_speed_kmh, predicted=False))
                ...
                if delta_t_hist > 0:
                    ...
                    if vx_pixels_per_time_unit < min_vx_for_prediction:
                        for lookahead_frames in [1, 2, 3]:
                            ...
                            predicted_x_at_crossing_time = ball_x_global + vx_pixels_per_time_unit * time_to_predict; predicted_timestamp = current_timestamp + time_to_predict
                            if predicted_x_at_crossing_time < self.center_x_global:
                                ...
                                if can_add_prediction: self.event_buffer_center_cross.append(EventRecord(predicted_x_at_crossing_time, predicted_timestamp, self.current_ball_speed_kmh, predicted=True)); break
    ...
    def _process_crossing_events(self):
        ...
        for event in temp_event_list:
            if not event.processed and not event.predicted and event.timestamp - self.last_committed_crossing_time >= self.EFFECTIVE_CROSSING_COOLDOWN_S: actual_event_to_commit = event; break
        if actual_event_to_commit:
            ...
            if len(self.collected_net_speeds) < self.max_net_speeds_to_collect:
                ...
                self.last_recorded_net_speed_kmh = event.speed_kmh; self.collected_net_speeds.append(event.speed_kmh); self.collected_relative_times.append(relative_time)
                self.last_committed_crossing_time = event.timestamp; self.ball_on_left_of_center = True; event.processed = True; processed_event_this_cycle = True; committed_event_ts = event.timestamp
            else: event.processed = True
        if not processed_event_this_cycle:
            predicted_event_to_commit = None
            for event in temp_event_list:
                if not event.processed and event.predicted and current_processing_time >= event.timestamp and event.timestamp - self.last_committed_crossing_time >= self.EFFECTIVE_CROSSING_COOLDOWN_S: predicted_event_to_commit = event; break
        ...
        if committed_event_ts > 0:
            for event_in_list in temp_event_list:
                if not event_in_list.processed and abs(event_in_list.timestamp - committed_event_ts) < self.EFFECTIVE_CROSSING_COOLDOWN_S / 2.0: event_in_list.processed = True
        for event_in_list in temp_event_list:
            if not event_in_list.processed and (current_processing_time - event_in_list.timestamp < 2.0): new_event_buffer.append(event_in_list)
    ...
    def _calculate_ball_speed(self):
        ...
        x1_glob, y1_glob, t1, _ = p1_glob; x2_glob, y2_glob, t2, _ = p2_glob
        dist_cm = self._calculate_real_distance_cm_area_based(x1_glob, y1_glob, x2_glob, y2_glob)
        delta_t = t2 - t1
        if delta_t > 0.0001:
            speed_cm_per_s = dist_cm / delta_t; speed_kmh = speed_cm_per_s * KMH_CONVERSION_FACTOR
            if self.current_ball_speed_kmh > 0: self.current_ball_speed_kmh = (1 - SPEED_SMOOTHING_FACTOR) * self.current_ball_speed_kmh + SPEED_SMOOTHING_FACTOR * speed_kmh
    ...
    def _calculate_real_distance_cm_area_based(self, x1_g, y1_g, x2_g, y2_g):
        ...
        pixel_distance = math.hypot(x2_g - x1_g, y2_g - y1_g); real_distance_cm = pixel_distance * pixel_to_cm_ratio
        return real_distance_cm
    def _generate_outputs_async(self):
        # ... same as v16
        if not self.collected_net_speeds: return
        speeds_copy = list(self.collected_net_speeds); times_copy = list(self.collected_relative_times); session_id_copy = self.count_session_id
    ...
    def _create_output_files(self, net_speeds, relative_times, session_id):
        ...
        if self.use_video_file and self.video_file_path:
            try: output_dir_path = os.path.dirname(self.video_file_path); video_filename_stem = os.path.splitext(os.path.basename(self.video_file_path))[0]; parts = video_filename_stem.split('_'); base_filename = f"{parts[0]}_{parts[1]}" if len(parts) >= 2 else video_filename_stem
            except Exception: self.use_video_file = False
        if not self.use_video_file or not self.video_file_path: timestamp_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S"); output_dir_path = f"{OUTPUT_DATA_FOLDER}/{timestamp_str}"; os.makedirs(output_dir_path, exist_ok=True); base_filename = f"speed_data_{timestamp_str}"
        avg_speed = sum(net_speeds) / len(net_speeds); max_speed = max(net_speeds); min_speed = min(net_speeds)
        chart_filename = os.path.join(output_dir_path, f'{base_filename}_chart.png'); txt_filename = os.path.join(output_dir_path, f'{base_filename}_data.txt'); csv_filename = os.path.join(output_dir_path, f'{base_filename}_data.csv')
        plt.figure(figsize=(12, 7)); plt.plot(relative_times, net_speeds, 'o-', label='Speed (km/h)'); plt.axhline(y=avg_speed, color='r', linestyle='--', label=f'Avg: {avg_speed:.1f} km/h')
        ...
        plt.title(f'Net Crossing Speeds - Session {session_id} - File: {base_filename}'); plt.xlabel('Relative Time (s)'); plt.ylabel('Speed (km/h)'); plt.grid(True); plt.legend(); plt.tight_layout(rect=[0, 0.03, 1, 0.95]); plt.savefig(chart_filename, dpi=150); plt.close()
        with open(txt_filename, 'w') as f: f.write(f"Net Speeds - Session {session_id}\n"); [f.write(f"{t:.2f}s: {s:.1f} km/h\n") for t, s in zip(relative_times, net_speeds)]; f.write(f"\nStats:\nAvg: {avg_speed:.1f}, Max: {max_speed:.1f}, Min: {min_speed:.1f}\n")
        with open(csv_filename, 'w', newline='') as f: writer = csv.writer(f); writer.writerow(['Session ID', 'Point', 'Time (s)', 'Speed (km/h)']); [writer.writerow([session_id, i+1, f"{t:.2f}", f"{s:.1f}"]) for i, (t, s) in enumerate(zip(relative_times, net_speeds))]
    ...
    def _draw_visualizations(self, display_frame, frame_data_obj: FrameData):
        # ... same as v16
        vis_frame = display_frame; is_full_draw = frame_data_obj.frame_counter % VISUALIZATION_DRAW_INTERVAL == 0
        if is_full_draw:
        ...
        if frame_data_obj.ball_position_in_roi and frame_data_obj.roi_sub_frame is not None:
            cx_roi, cy_roi = frame_data_obj.ball_position_in_roi; cv2.circle(frame_data_obj.roi_sub_frame, (cx_roi, cy_roi), 5, BALL_COLOR_BGR, -1)
            ...
            cx_global_vis = cx_roi + self.roi_start_x; cy_global_vis = cy_roi + self.roi_top_y; cv2.circle(vis_frame, (cx_global_vis, cy_global_vis), 8, BALL_COLOR_BGR, -1)
        ...
        if frame_data_obj.viz_text: cv2.putText(vis_frame, frame_data_obj.viz_text, (10, self.frame_height - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        if self.debug_mode and frame_data_obj.debug_display_text: cv2.putText(vis_frame, frame_data_obj.debug_display_text, (10, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 0), 1)
        return vis_frame
    def _check_timeout_and_reset(self):
        # ... same as v16
        if time.monotonic() - self.last_detection_timestamp > self.detection_timeout_s:
            self.trajectory.clear(); self.area_history.clear(); self.current_ball_speed_kmh = 0

def main():
    parser = argparse.ArgumentParser(description='Ping Pong Speed Tracker v17 (Persistent Calibration)')
    # ... (命令列參數與 v16 相同) ...
