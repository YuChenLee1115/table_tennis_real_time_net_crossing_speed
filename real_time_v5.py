...
AUTO_STOP_AFTER_COLLECTION = True  # 收集完速度後自動停止程序
...
class PingPongSpeedTracker:
    ...
    def check_center_crossing(self, ball_x, timestamp):
        ...
        if record_crossing and self.ball_speed > 0:
            ...
            if len(self.net_speeds) >= MAX_NET_SPEEDS and not self.output_generated:
                ...
                # 如果設定為收集完成後自動停止，設置退出標記
                if AUTO_STOP_AFTER_COLLECTION:
                    print("已收集完 30 個速度值，程序將自動停止。")
    ...
    def generate_outputs(self):
        ...
        plt.title('Table Tennis Net Speed Record')
        plt.xlabel('Count')
        ...
        with open(txt_filename, 'w') as f:
            f.write("Table Tennis Net Speed Record (km/h)\n")
            f.write("----------------------------------\n")
        ...
        print(f"輸出已保存到 {OUTPUT_FOLDER} 資料夾，時間戳記: {timestamp}")
    ...
    def run(self):
        ...
        while True:
            ...
            if not ret:
                ...
                # 如果影片結束但尚未收集完30個數據點，仍然生成輸出
                if len(self.net_speeds) > 0 and not self.output_generated:
                    self.generate_outputs()
                break
...
def main():
    ...
    parser.add_argument('--direction', type=str, default=NET_CROSSING_DIRECTION, 
                        ...
                        help='記錄球經過網中心的方向')
    parser.add_argument('--auto_stop', type=bool, default=AUTO_STOP_AFTER_COLLECTION, 
                        help='是否在收集完30個速度後自動停止程序')
