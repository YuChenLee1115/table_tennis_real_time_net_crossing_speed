#!/usr/bin/env python3
# 桌球速度追蹤器 - GPU 加速版 (基於 v7)
# 使用 Metal 和 PyOpenCL 支援 Apple M2 Pro
import cv2
import numpy as np
import time
import datetime
...
try:
    ...
    print("PyOpenCL 可用，將用於 GPU 加速")
except ImportError:
    OPENCL_AVAILABLE = False
    print("PyOpenCL 未安裝，將使用 CPU 處理")
    print("提示: 可通過 'pip install pyopencl' 安裝")

# 檢查 OpenCV 是否支援 Metal 或 OpenCL
CV_METAL_AVAILABLE = hasattr(cv2, 'CAP_VIDEOCAPTURE_METAL')
CV_OPENCL_AVAILABLE = cv2.ocl.haveOpenCL()

if CV_METAL_AVAILABLE:
    print("OpenCV 支援 Metal 後端")
elif CV_OPENCL_AVAILABLE:
    print("OpenCV 支援 OpenCL 後端")
    # 啟用 OpenCV 中的 OpenCL 加速
    cv2.ocl.setUseOpenCL(True)
    print(f"OpenCL 已啟用: {cv2.ocl.useOpenCL()}")
else:
    print("OpenCV 未編譯支援 Metal/OpenCL，將使用 CPU 處理")
...
DEFAULT_ROI_START_RATIO = 0.35  # ROI 區域開始比例 (左側)
DEFAULT_ROI_END_RATIO = 0.65  # ROI 區域結束比例 (右側)
...
class PingPongSpeedTracker:
    ...
    def init_gpu_acceleration(self):
        ...
        self.opencl_ctx = None
        self.opencl_queue = None
        
        # 嘗試設置 Metal 加速
        if CV_METAL_AVAILABLE and self.use_gpu:
            try:
                self.cap.set(cv2.CAP_PROP_VIDEOCAPTURE_METAL, 1)
                print("Metal 加速已啟用於視頻捕獲")
            except Exception as e:
                print(f"Metal 加速設置失敗: {e}")
        
        # 嘗試初始化 OpenCL
        if OPENCL_AVAILABLE and self.use_gpu:
            try:
                ...
                if platforms:
                    ...
                    platform = apple_platform if apple_platform else platforms[0]
                    devices = platform.get_devices(device_type=cl.device_type.GPU)
                    
                    if devices:
                        # 創建 OpenCL 上下文和命令隊列
                        self.opencl_ctx = cl.Context([devices[0]])
                        self.opencl_queue = cl.CommandQueue(self.opencl_ctx)
                        self.gpu_available = True
                        
                        device_name = devices[0].name
                        print(f"OpenCL GPU 加速已啟用: {device_name}")
                        
                        # 編譯 OpenCL 程序
                        self.init_opencl_kernels()
                    else:
                        print("未找到 GPU 設備，將使用 CPU")
                else:
                    print("未找到 OpenCL 平台，將使用 CPU")
            except Exception as e:
                print(f"OpenCL 初始化失敗: {e}")
                print("將使用 CPU 處理")
    
    def init_opencl_kernels(self):
        """初始化 OpenCL 內核程序"""
        ...
        try:
            # 定義 OpenCL 内核程序代碼
            opencl_code = """
            __kernel void absdiff(__global const uchar* a, 
                ...
                if (x < width && y < height) {
                    int idx = y * width + x;
                    result[idx] = (a[idx] > b[idx]) ? (a[idx] - b[idx]) : (b[idx] - a[idx]);
                }
            }
            
            __kernel void bitwise_and(__global const uchar* a, 
                                     __global const uchar* b, 
                                     __global uchar* result,
                                     const int width, 
                                     const int height) {
            ...
            __kernel void threshold(__global const uchar* src, 
                                   __global uchar* dst,
                                   const int width, 
                                   const int height,
                                   const uchar thresh_value) {
                int x = get_global_id(0);
                int y = get_global_id(1);
                
                if (x < width && y < height) {
                    int idx = y * width + x;
                    dst[idx] = (src[idx] > thresh_value) ? 255 : 0;
                }
            }
            """
            
            # 編譯 OpenCL 程序
            self.opencl_program = cl.Program(self.opencl_ctx, opencl_code).build()
            print("OpenCL 程序編譯成功")
        except Exception as e:
            print(f"OpenCL 程序編譯失敗: {e}")
            self.gpu_available = False
    ...
    def preprocess_frame(self, frame):
        ...
        # 轉換為灰階
        if CV_OPENCL_AVAILABLE and self.use_gpu:
            # 使用 UMat (OpenCL 加速) 進行灰階轉換
            roi_umat = cv2.UMat(roi)
            gray_umat = cv2.cvtColor(roi_umat, cv2.COLOR_BGR2GRAY)
            gray = gray_umat.get()  # 轉回 CPU 記憶體
        else:
            # 使用普通 CPU 版本
    ...
    def detect_fmo_gpu(self):
        ...
        # 創建緩衝區
        mf = cl.mem_flags
        f1_buf = cl.Buffer(self.opencl_ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=f1)
        f2_buf = cl.Buffer(self.opencl_ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=f2)
        f3_buf = cl.Buffer(self.opencl_ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=f3)
        
        diff1_buf = cl.Buffer(self.opencl_ctx, mf.WRITE_ONLY, f1.nbytes)
        diff2_buf = cl.Buffer(self.opencl_ctx, mf.WRITE_ONLY, f1.nbytes)
        mask_buf = cl.Buffer(self.opencl_ctx, mf.WRITE_ONLY, f1.nbytes)
        thresh_buf = cl.Buffer(self.opencl_ctx, mf.WRITE_ONLY, f1.nbytes)
        
        # 執行 absdiff 計算
        absdiff = self.opencl_program.absdiff
        absdiff.set_args(f1_buf, f2_buf, diff1_buf, np.int32(width), np.int32(height))
        cl.enqueue_nd_range_kernel(self.opencl_queue, absdiff, (width, height), None)
        
        absdiff.set_args(f2_buf, f3_buf, diff2_buf, np.int32(width), np.int32(height))
        cl.enqueue_nd_range_kernel(self.opencl_queue, absdiff, (width, height), None)
        
        # 執行 bitwise_and 計算
        bitwise_and = self.opencl_program.bitwise_and
        bitwise_and.set_args(diff1_buf, diff2_buf, mask_buf, np.int32(width), np.int32(height))
        cl.enqueue_nd_range_kernel(self.opencl_queue, bitwise_and, (width, height), None)
        
        # 執行閾值處理
        threshold = self.opencl_program.threshold
        threshold.set_args(mask_buf, thresh_buf, np.int32(width), np.int32(height), np.uint8(THRESHOLD_VALUE))
        cl.enqueue_nd_range_kernel(self.opencl_queue, threshold, (width, height), None)
        
        # 讀取結果
        result = np.empty_like(f1)
        cl.enqueue_copy(self.opencl_queue, result, thresh_buf)
        
        # 同步等待操作完成
        self.opencl_queue.finish()
        
        # 使用 CPU 進行形態學操作 (難以 GPU 優化)
        opening = cv2.morphologyEx(result, cv2.MORPH_OPEN, self.opening_kernel)
    ...
    def detect_fmo(self):
        ...
        """
        # 如果 GPU 可用，使用 GPU 加速版本
        if self.gpu_available and self.use_gpu:
            try:
                return self.detect_fmo_gpu()
            except Exception as e:
                print(f"GPU 處理錯誤，切換到 CPU: {e}")
        ...
        # 計算連續幀之間的差異
        if CV_OPENCL_AVAILABLE and self.use_gpu:
            # 使用 UMat (OpenCL 加速) 進行圖像處理
            f1_umat = cv2.UMat(f1)
            f2_umat = cv2.UMat(f2)
            f3_umat = cv2.UMat(f3)
            
            diff1 = cv2.absdiff(f1_umat, f2_umat)
            diff2 = cv2.absdiff(f2_umat, f3_umat)
    ...
    def detect_ball(self, roi, mask):
        ...
        if CV_OPENCL_AVAILABLE and self.use_gpu:
            ...
            contours_output = cv2.findContours(mask_umat, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cnts = contours_output[0].get() if len(contours_output) == 2 else contours_output[1].get()
        else:
            # 使用普通 CPU 版本
    ...
    def draw_visualizations(self, frame, roi, ball_position=None, ball_contour=None):
        ...
        if CV_OPENCL_AVAILABLE and self.use_gpu:
            ...
            # 繪製 ROI 邊界
            cv2.line(frame_umat, (self.roi_start_x, 0), (self.roi_start_x, self.frame_height), ROI_COLOR, 2)
            cv2.line(frame_umat, (self.roi_end_x, 0), (self.roi_end_x, self.frame_height), ROI_COLOR, 2)
            cv2.line(frame_umat, (0, self.roi_end_y), (self.frame_width, self.roi_end_y), ROI_COLOR, 2)
    
            # 繪製中心線
            cv2.line(frame_umat, (self.center_x, 0), (self.center_x, self.frame_height), CENTER_LINE_COLOR, 2)
            
            # 轉回 CPU 記憶體
            cv2.UMat.get(frame_umat, frame)
        else:
            # 繪製 ROI 邊界
        ...
        if self.processing_times:
            avg_proc_time = sum(self.processing_times) / len(self.processing_times) * 1000
            cv2.putText(
                frame,
                f"Proc time: {avg_proc_time:.1f} ms",
                (10, 230),
                cv2.FONT_HERSHEY_SIMPLEX,
                FONT_SCALE,
                (255, 255, 0),
        ...
        # —— 顯示是否啟用 GPU ——
        gpu_status = "ON" if (self.gpu_available or CV_OPENCL_AVAILABLE) and self.use_gpu else "OFF"
        gpu_color = (0, 255, 0) if gpu_status == "ON" else (0, 0, 255)
        cv2.putText(
            frame,
            f"GPU: {gpu_status}",
            (10, 270),
    ...
    def run(self):
        """執行主循環"""
        print("=== 乒乓球速度追蹤器 GPU 加速版 ===")
        print("按下空白鍵開始/停止計數")
        print("按下 'q' 或 ESC 鍵退出程序")
        print(f"使用 GPU 加速: {(self.gpu_available or CV_OPENCL_AVAILABLE) and self.use_gpu}")
        ...
        while True:
            ...
            self.processing_times.append(proc_end_time - proc_start_time)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='乒乓球速度追蹤器 (GPU 加速版)')
