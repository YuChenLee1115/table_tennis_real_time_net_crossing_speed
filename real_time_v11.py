#!/usr/bin/env python3
# 乒乓球速度追蹤系統 v11
# 方向過濾優化版本：專注於左至右方向的乒乓球追蹤、優化 ROI 設定、移動向量分析
...
DEFAULT_ROI_START_RATIO = 0.2  # ROI 區域開始比例 (左側)，擴大左側範圍從0.4減少到0.2
DEFAULT_ROI_END_RATIO = 0.7  # ROI 區域結束比例 (右側)，擴大右側範圍從0.6增加到0.7
DEFAULT_ROI_BOTTOM_RATIO = 0.8  # ROI 區域底部比例 (排除底部 10%)
MAX_TRAJECTORY_POINTS = 80  # 最大軌跡點數

# 方向過濾參數 (新增)
DIRECTION_FILTER_ENABLED = True  # 是否啟用方向過濾 
DIRECTION_THRESHOLD = 5  # 用於確定移動方向的閾值 (像素差異)
MOTION_SMOOTHING = 3  # 運動方向平滑因子，數值越大，方向判斷越穩定

# 中心線偵測參數
CENTER_LINE_WIDTH = 20  # 中心線寬度 (像素)
CENTER_DETECTION_COOLDOWN = 0.15  # 中心點偵測冷卻時間 (秒)
MAX_NET_SPEEDS = 100  # 紀錄的最大網中心速度數量
NET_CROSSING_DIRECTION = 'left_to_right'  # 強制只記錄左到右方向的速度
...
CLOSING_KERNEL_SIZE = (15, 15)  # 閉運算內核大小
...
DIRECTION_OK_COLOR = (0, 255, 0)  # 方向正確時的顏色 (BGR)
DIRECTION_WRONG_COLOR = (0, 0, 255)  # 方向錯誤時的顏色 (BGR)
...
class FrameData:
    ...
    def __init__(self, frame=None, roi=None, ball_position=None, ball_contour=None, ball_speed=0, 
        ...
        self.direction_status = direction_status  # 新增：存儲方向狀態 (True=左到右, False=其他方向, None=未知)
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM, 
                 ...
                 debug_mode=DEBUG_MODE, direction_filter=DIRECTION_FILTER_ENABLED):
        """初始化乒乓球速度追蹤器"""
        # 設置調試模式
        ...
        self.motion_vectors = deque(maxlen=MOTION_SMOOTHING)  # 儲存最近的運動向量
        self.current_direction = None  # None = 未知, True = 左到右, False = 右到左
        ...
        # 方向過濾統計 (新增)
        self.direction_filter_stats = {
            'left_to_right': 0,  # 左到右計數
            'right_to_left': 0,  # 右到左計數
            'unknown': 0,        # 無法確定計數
            'filtered_frames': 0 # 被過濾的總幀數
        }
    ...
    def _setup_precalculated_elements(self):
        ...
        # 建立指導訊息文字
        self.instruction_text = "Press SPACE to toggle counting, D for debug mode, F to toggle direction filter, ESC/q to quit"
    ...
    def detect_fmo(self):
        """
        使用快速移動物體偵測 (FMO) 方法偵測移動物體 - 加入方向過濾功能
        ...
        if self.direction_filter:
            ...
            left_half1 = diff1[:, :width//2]
            right_half1 = diff1[:, width//2:]
            left_half2 = diff2[:, :width//2]
            right_half2 = diff2[:, width//2:]
            
            # 計算各區域差異總和
            left_sum1 = np.sum(left_half1)
            right_sum1 = np.sum(right_half1)
            left_sum2 = np.sum(left_half2)
            right_sum2 = np.sum(right_half2)
            
            # 判斷主要運動方向
            left_to_right = (left_sum1 > right_sum1 + DIRECTION_THRESHOLD and 
                            right_sum2 > left_sum2 + DIRECTION_THRESHOLD)
            right_to_left = (right_sum1 > left_sum1 + DIRECTION_THRESHOLD and 
                            left_sum2 > right_sum2 + DIRECTION_THRESHOLD)
            
            # 更新向量列表和當前方向
            if left_to_right:
                self.motion_vectors.append(1)  # 1 表示左到右
                self.direction_filter_stats['left_to_right'] += 1
            elif right_to_left:
                self.motion_vectors.append(-1)  # -1 表示右到左
                self.direction_filter_stats['right_to_left'] += 1
            else:
                self.motion_vectors.append(0)  # 0 表示無法確定
                self.direction_filter_stats['unknown'] += 1
            
            # 根據最近的向量平均值確定方向
            if len(self.motion_vectors) > 0:
                avg_direction = sum(self.motion_vectors) / len(self.motion_vectors)
                if avg_direction > 0.3:  # 傾向於左到右
                    self.current_direction = True
                elif avg_direction < -0.3:  # 傾向於右到左
                    self.current_direction = False
                    # 如果啟用了方向過濾且方向是右到左，返回空遮罩
                    self.direction_filter_stats['filtered_frames'] += 1
                    return np.zeros_like(diff1)
                else:
                    # 方向不明確，保持原有方向不變
                    pass
            ...
            if potential_balls:
                ...
                if best_ball:
                    ...
                    if len(self.trajectory) >= 2:
                        prev_x = self.trajectory[-2][0]
                        is_left_to_right = cx_orig > prev_x
                        is_right_to_left = cx_orig < prev_x
                        
                        # 更新方向狀態
                        if is_left_to_right and not is_right_to_left:
                            self.current_direction = True
                        elif is_right_to_left and not is_left_to_right:
                            self.current_direction = False
                    
                    if self.debug_mode:
                        print(f"偵測到球：位置({cx_orig}, {cy})，面積:{best_ball['area']:.1f}，圓度:{best_ball['circularity']:.3f}，方向:{'左到右' if self.current_direction == True else '右到左' if self.current_direction == False else '未知'}")
    ...
    def _select_best_ball(self, potential_balls):
        ...
        # 方向過濾：如果啟用且有明確方向，過濾掉不符合當前方向的球
        if self.direction_filter and self.current_direction is not None:
            # 首先按距離排序
            for ball in potential_balls:
                ...
                # 如果我們在追蹤左到右的移動，但這個球在之前的球的左側，降低其優先級
                if self.current_direction == True and orig_x < last_x:  # 預期左到右，但球在左側
                    ball['distance'] *= 1.5  # 增加距離權重，降低優先級
                
                # 如果我們在追蹤右到左的移動，但這個球在之前的球的右側，降低其優先級
                elif self.current_direction == False and orig_x > last_x:  # 預期右到左，但球在右側
                    ball['distance'] *= 1.5  # 增加距離權重，降低其優先級
                
                # 如果距離太遠，可能是不相關的球
                if ball['distance'] > self.frame_width * 0.3:  # 超過畫面寬度的30%視為太遠
        ...
        for ball in potential_balls:
            ...
            # 如果啟用方向過濾，增加方向一致性的權重
            if self.direction_filter:
                ball['score'] = distance_score * 0.4 + consistency_score * 0.4 + circularity_score * 0.2
            else:
    ...
    def toggle_direction_filter(self):
        """切換方向過濾狀態"""
        self.direction_filter = not self.direction_filter
        print(f"方向過濾: {'開啟' if self.direction_filter else '關閉'}")
        
        # 如果關閉方向過濾，重置方向相關變數
        if not self.direction_filter:
    ...
    def _record_potential_crossing(self, ball_x, timestamp):
        ...
        # 實際穿越檢測 - 由於我們只關心左到右的方向，移除其他判斷
        record_crossing = False
        
        # 只記錄左到右方向 (無論 NET_CROSSING_DIRECTION 參數如何設置)
        ...
        if len(self.trajectory) >= 2 and direction > 0:  # 只預測左到右的穿越
            # 計算當前速度和方向
            ...
            if curr_t > last_t:  # 避免除以零
                ...
                # 檢查是否會從左向右穿過中心線
                if x_velocity > 0 and ball_x < self.center_x and ball_x + x_velocity * PREDICTION_LOOKAHEAD / self.fps >= self.center_x:
                    will_cross = True
                    
                    # 估計穿越時間
                    time_to_cross = (self.center_x - ball_x) / x_velocity if x_velocity != 0 else 0
    ...
    def generate_outputs(self):
        ...
        # 方向過濾統計報告
        if self.debug_mode and self.direction_filter:
            print("\n方向過濾統計報告:")
            print(f"- 左到右方向偵測: {self.direction_filter_stats['left_to_right']} 次")
            print(f"- 右到左方向偵測: {self.direction_filter_stats['right_to_left']} 次")
            print(f"- 方向不明確: {self.direction_filter_stats['unknown']} 次")
            print(f"- 被過濾的幀數: {self.direction_filter_stats['filtered_frames']} 幀")
            total_frames = sum(self.direction_filter_stats.values()) - self.direction_filter_stats['filtered_frames']
            if total_frames > 0:
                filter_rate = self.direction_filter_stats['filtered_frames'] / total_frames * 100
                print(f"- 過濾率: {filter_rate:.1f}%")
    ...
    def _generate_chart(self, output_dir, timestamp):
        ...
        # 設置圖表標題和標籤
        plt.title(f'Table Tennis Left-to-Right Net Speed Record', fontsize=16, pad=20)  # 更新標題以反映方向
        ...
        # 添加圖表說明文字
        plt.figtext(0.02, 0.02, f"Direction: Left-to-Right | Total: {len(plot_speeds)} | Max: {max_speed:.1f} km/h | Min: {min_speed:.1f} km/h", 
    ...
    def _generate_txt(self, output_dir, timestamp):
        ...
        with open(txt_filename, 'w') as f:
            f.write(f"Table Tennis Left-to-Right Net Speed Record (km/h) - Session {self.count_session}\n")
            f.write("----------------------------------\n")
            ...
            if self.direction_filter:
                f.write("\n----------------------------------\n")
                f.write("Direction Filter: Enabled (Left-to-Right Only)\n")
    ...
    def _generate_csv(self, output_dir, timestamp):
        ...
        with open(csv_filename, 'w', newline='') as f:
            ...
            if self.direction_filter:
                ...
                csv_writer.writerow(["Direction Filter", "Left-to-Right Only"])
    ...
    def draw_visualizations(self, frame, frame_data):
        ...
        # —— 顯示方向過濾狀態 ——
        filter_status = "ON" if self.direction_filter else "OFF"
        filter_color = DIRECTION_OK_COLOR if self.direction_filter else DIRECTION_WRONG_COLOR
        cv2.putText(
            vis_layer,
            f"Direction Filter: {filter_status}",
            (10, 110),
            cv2.FONT_HERSHEY_SIMPLEX,
            FONT_SCALE,
            filter_color,
        ...
        # —— 顯示當前移動方向 ——
        if frame_data.direction_status is not None:
            direction_text = "Left to Right" if frame_data.direction_status == True else "Right to Left" if frame_data.direction_status == False else "Unknown"
            direction_color = DIRECTION_OK_COLOR if frame_data.direction_status == True else DIRECTION_WRONG_COLOR
            cv2.putText(
                vis_layer,
        ...
        count_color = DIRECTION_OK_COLOR if frame_data.is_counting else DIRECTION_WRONG_COLOR
        cv2.putText(
            vis_layer,
    ...
    def process_frame(self, frame):
        ...
        frame_data = FrameData(
            ...
            direction_status=self.current_direction  # 添加方向狀態
        )
        
        # 添加調試信息
        if self.debug_mode:
            if self.direction_filter:
                frame_data.display_text = f"L→R:{self.direction_filter_stats['left_to_right']} R→L:{self.direction_filter_stats['right_to_left']} Filtered:{self.direction_filter_stats['filtered_frames']}"
    ...
    def display_thread_function(self):
        ...
        while self.running:
            try:
                ...
                elif key == ord('f'):  # 'f' 鍵，切換方向過濾
                    self.key_pressed = ord('f')
    ...
    def run(self):
        """執行主循環 - macOS 兼容版本"""
        print("=== 乒乓球速度追蹤器 v11 (方向過濾優化版) ===")
        print("按下空白鍵開始/停止計數")
        print("按下 'd' 鍵切換調試模式")
        print("按下 'f' 鍵切換方向過濾")
        print("按下 'q' 或 ESC 鍵退出程序")
        print(f"方向過濾: {'啟用' if self.direction_filter else '禁用'}")
        print(f"ROI 設定: 左側 {self.roi_start_x}px, 右側 {self.roi_end_x}px (比例 {DEFAULT_ROI_START_RATIO}-{DEFAULT_ROI_END_RATIO})")
...
def main():
    ...
    global DIRECTION_FILTER_ENABLED, DEFAULT_ROI_START_RATIO, DEFAULT_ROI_END_RATIO
    ...
    parser.add_argument('--roi_start', type=float, default=DEFAULT_ROI_START_RATIO,
                        help='ROI 區域左側起始比例 (0.0-1.0)')
    parser.add_argument('--roi_end', type=float, default=DEFAULT_ROI_END_RATIO,
                        help='ROI 區域右側結束比例 (0.0-1.0)')
    parser.add_argument('--no_filter', action='store_true', help='禁用方向過濾')
    args = parser.parse_args()

    # 設置全局參數
    NET_CROSSING_DIRECTION = 'left_to_right'  # 強制只記錄左到右方向
    ...
    DIRECTION_FILTER_ENABLED = not args.no_filter
    ...
    if args.video:
        tracker = PingPongSpeedTracker(
            ...
            direction_filter=DIRECTION_FILTER_ENABLED
        )
    else:
        tracker = PingPongSpeedTracker(
            args.camera, 
