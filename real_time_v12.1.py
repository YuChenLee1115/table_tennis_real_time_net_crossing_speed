#!/usr/bin/env python3
# 乒乓球速度追蹤系統 v11.2 (ROI R-L Avg Speed - Improved 60FPS Robustness)
...
DEFAULT_DETECTION_TIMEOUT = 0.2 # Increased slightly
...
# Slightly larger kernels for potentially larger inter-frame movement/blur
OPENING_KERNEL_SIZE_FMO = (15, 15) # Was (12,12)
CLOSING_KERNEL_SIZE_FMO = (30, 30) # Was (25,25)
THRESHOLD_VALUE_FMO = 9 # Otsu is preferred, this is fallback

# Ball Detection Parameters
MIN_BALL_AREA_PX = 10 # Might need adjustment based on distance and blur
MAX_BALL_AREA_PX = 10000
MIN_BALL_CIRCULARITY = 0.28 # Lowered from 0.32 for motion blur tolerance
MAX_DIST_FROM_LAST_BALL_RATIO = 0.35 # Increased from 0.2 for larger jumps at lower FPS
...
# FPS Calculation
FPS_SMOOTHING_FACTOR = 0.3
MAX_FRAME_TIMES_FPS_CALC = 20

# Transit Tracking
# Number of consecutive frames ball can be missed during active transit before finalizing
MAX_CONSECUTIVE_MISSES_IN_TRANSIT = 3 # NEW - Crucial for 60FPS
MIN_POINTS_FOR_VALID_TRANSIT = 3 # NEW - Minimum data points for a transit to be considered valid
...
TRANSIT_SPEED_TEXT_COLOR_BGR = (255, 0, 0)
FONT_SCALE_VIS = 1
...
class FrameData:
    ...
    def __init__(self, frame=None, roi_sub_frame=None, ball_position_in_roi=None,
        ...
        self.last_recorded_transit_speed_kmh = last_recorded_transit_speed_kmh
...
class FrameReader:
    ...
    def __init__(self, video_source, target_fps, use_video_file, frame_width, frame_height):
        ...
        # Try to use GStreamer backend for potentially better performance on some systems
        # or AVFoundation on macOS by default. CAP_FFMPEG can also be good.
        # Auto is usually fine: cv2.VideoCapture(self.video_source)
        self.cap = cv2.VideoCapture(self.video_source, cv2.CAP_AVFOUNDATION) # macOS default
    ...
    def _configure_capture(self, frame_width, frame_height):
        if not self.use_video_file:
            ...
            self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG')) # Might improve FPS for some webcams
    ...
    def _read_frames(self):
        while self.running:
            if not self.frame_queue.full():
                ...
                time.sleep(1.0 / (self.target_fps * 2)) # Avoid busy-waiting
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM,
        ...
        self.missed_ball_frames_in_transit = 0 # NEW counter
    ...
    def _preprocess_frame(self, frame):
        ...
        gray_roi_blurred = cv2.GaussianBlur(gray_roi, (5, 5), 0) # Standard blur
    ...
    def _detect_fmo(self):
        ...
        # Using a copy of deque items for safety, though not strictly necessary if only reading
        f1, f2, f3 = list(self.prev_frames_gray_roi)[-3], list(self.prev_frames_gray_roi)[-2], list(self.prev_frames_gray_roi)[-1]
        
        diff1 = cv2.absdiff(f1, f2)
        diff2 = cv2.absdiff(f2, f3)
        motion_mask = cv2.bitwise_and(diff1, diff2)
        
        try: # OTSU is generally good
            _, thresh_mask = cv2.threshold(motion_mask, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        except cv2.error: # Fallback if OTSU fails (e.g., all black image)
        ...
        else: opened_mask = thresh_mask # Skip if kernel is (0,0)
    ...
    def _detect_ball_in_roi(self, motion_mask_roi):
        ...
        self.last_detection_timestamp = time.monotonic() # General system time for timeout
        self.trajectory.append((cx_global, cy_global, current_timestamp)) # Timestamp used by transit logic
        
        if self.debug_mode:
             debug_circ = best_ball_info['circularity']
             # print(f"Ball: ROI({cx_roi},{cy_roi}), G({cx_global},{cy_global}), A:{best_ball_info['area']:.0f}, C:{debug_circ:.2f}, T:{current_timestamp:.3f}")
    ...
    def _select_best_ball_candidate(self, candidates):
        ...
        # Filter by circularity first
        circular_candidates = [b for b in candidates if b['circularity'] >= MIN_BALL_CIRCULARITY]
        if not circular_candidates:
            if self.debug_mode and candidates: print(f"DEBUG: No candidates met MIN_BALL_CIRCULARITY {MIN_BALL_CIRCULARITY}. Best was {max(candidates, key=lambda b: b['circularity'])['circularity']:.2f}")
            return None # Stricter: if nothing is circular enough, it's not a ball

        if not self.trajectory: # No history, pick most circular one from the good ones
            return max(circular_candidates, key=lambda b: b['circularity'])

        last_x_global, last_y_global, _ = self.trajectory[-1]
        for ball_info in circular_candidates: # Only process sufficiently circular candidates
            ...
            # Penalize balls too far from last known position (increased threshold)
            if distance > self.frame_width * MAX_DIST_FROM_LAST_BALL_RATIO:
            ...
            if len(self.trajectory) >= 2:
                ...
                if mag_hist > 1e-6 and mag_curr > 1e-6: # Avoid division by zero for tiny movements
                    cosine_similarity = dot_product / (mag_hist * mag_curr)
                else: cosine_similarity = 0
                consistency_score = max(0, cosine_similarity) # We prefer forward consistency
            ball_info['consistency'] = consistency_score
        
        # Scoring: Higher is better. prioritize consistency and proximity for trajectory continuation.
        for ball_info in circular_candidates:
            # Increased weight for consistency, reduced for distance penalty (which is now harsher upfront)
            score = (0.2 / (1.0 + ball_info['distance_from_last'])) + \
                    (0.6 * ball_info['consistency']) + \
                    (0.2 * ball_info['circularity']) # Circularity already pre-filtered
            ball_info['score'] = score
        
        return max(circular_candidates, key=lambda b: b['score'])
    ...
    def toggle_counting(self):
        ...
        if self.is_counting_active:
            ...
            self.first_ball_event_timestamp_in_session = None
            self.output_generated_for_session = False
            self.is_tracking_r_to_l_transit = False # Reset any ongoing transit
            self.current_r_to_l_transit_data = []
            self.missed_ball_frames_in_transit = 0
            ...
            self.output_generated_for_session = True # Mark generated to avoid re-gen if toggled fast
    ...
    def _update_r_to_l_transit_tracking(self, cx_global, cy_global, current_timestamp, current_speed_kmh):
        ...
        if len(self.trajectory) < 2:
            if self.is_tracking_r_to_l_transit:
                self._finalize_r_to_l_transit(current_timestamp, "ball_trajectory_lost_short")
            return

        prev_x_global, _, _ = self.trajectory[-2]
        
        # Define movement with a small threshold to avoid jitter effects.
        # Increased threshold for direction change detection.
        movement_threshold_px = 3 # Pixels
        is_moving_left = (cx_global < prev_x_global - movement_threshold_px)
        is_moving_right = (cx_global > prev_x_global + movement_threshold_px)

        if is_moving_left:
            if not self.is_tracking_r_to_l_transit:
                # Start new transit if ball is within ROI and moving left
                if self.roi_start_x <= cx_global <= self.roi_end_x: # Ensure it's within ROI X bounds
                    ...
                    self.current_r_to_l_transit_data = [] # Clear for new transit
            
            if self.is_tracking_r_to_l_transit: # If already tracking or just started
                self.current_r_to_l_transit_data.append({'x': cx_global, 'y': cy_global, 't': current_timestamp})
            
                # Exit condition: ball moves out of ROI on the left.
                # Make this condition slightly outside ROI to ensure it truly exited.
                if cx_global < self.roi_start_x: # Ball has crossed the left boundary of ROI
                    if self.debug_mode: print(f"DEBUG: R-to-L transit MET EXIT LEFT ROI (x={cx_global} < ROI_start={self.roi_start_x})")
        ...
        else: # Stalled or minor vertical movement
            if self.is_tracking_r_to_l_transit:
                # Ball stalled, keep adding point. Timeout or other conditions will finalize.
                self.current_r_to_l_transit_data.append({'x': cx_global, 'y': cy_global, 't': current_timestamp})


    def _finalize_r_to_l_transit(self, end_timestamp_of_last_data_point, reason):
        # end_timestamp_of_last_data_point should be the timestamp of the last known good ball position in this transit

        if not self.is_tracking_r_to_l_transit or len(self.current_r_to_l_transit_data) < MIN_POINTS_FOR_VALID_TRANSIT:
            if self.debug_mode and self.is_tracking_r_to_l_transit:
                print(f"DEBUG: Finalize '{reason}' called, but insufficient transit data ({len(self.current_r_to_l_transit_data)} pts, need {MIN_POINTS_FOR_VALID_TRANSIT}). Resetting.")
            self.is_tracking_r_to_l_transit = False
            self.current_r_to_l_transit_data = []
            self.missed_ball_frames_in_transit = 0
            return
        ...
        end_point_data = self.current_r_to_l_transit_data[-1] # Use actual last point of this transit
        ...
        # Use the provided end_timestamp which should be from the last valid data point of the transit.
        # If the reason is something like "exited_left_roi_boundary", this end_timestamp is crucial.
        delta_t = end_point_data['t'] - start_point_data['t'] # Duration of the observed transit path

        avg_speed_kmh = 0
        if delta_t > 0.001: # Min duration for meaningful speed
            avg_speed_cm_per_s = total_dist_cm / delta_t
            avg_speed_kmh = avg_speed_cm_per_s * KMH_CONVERSION_FACTOR
        else:
            if self.debug_mode: print(f"DEBUG: Transit duration {delta_t:.4f}s too short. Speed set to 0.")

        # Reset transit state BEFORE checking collection limits or generating output
        # This is important to allow a new transit to start immediately if conditions are met
        current_transit_copy = list(self.current_r_to_l_transit_data) # For debug or logging if needed
        self.is_tracking_r_to_l_transit = False
        self.current_r_to_l_transit_data = []
        self.missed_ball_frames_in_transit = 0
        ...
        if avg_speed_kmh > 1.0: # Only record speeds that are somewhat meaningful (e.g. > 1 km/h)
            if len(self.collected_transit_speeds) >= self.max_transit_speeds_to_collect:
                if not self.output_generated_for_session:
                    print(f"Max transit speeds ({self.max_transit_speeds_to_collect}) collected. Further ignored.")
                    self._generate_outputs_async()
            ...
            print(f"ROI R-L Transit #{len(self.collected_transit_speeds)}: Avg Speed {avg_speed_kmh:.1f} km/h @ Rel.T {relative_time:.2f}s (Dur: {delta_t:.2f}s, Pts: {len(current_transit_copy)}, Reason: {reason})")
            ...
            if len(self.collected_transit_speeds) >= self.max_transit_speeds_to_collect and not self.output_generated_for_session:
             ...
             print(f"DEBUG: Calculated transit speed {avg_speed_kmh:.1f} km/h too low (reason: {reason}). Discarding.")


    def _calculate_ball_speed(self): # INSTANTANEOUS speed
        if len(self.trajectory) < 2:
            self.current_ball_speed_kmh = 0
            return
        ...
        delta_t = t2 - t1
        if delta_t > 1e-5: # Avoid division by zero/very small dt
        ...
        else: self.current_ball_speed_kmh *= (1 - SPEED_SMOOTHING_FACTOR) # Decay
    ...
    def _create_output_files(self, transit_speeds, relative_times, session_id):
        ...
        output_dir_path = f"{OUTPUT_DATA_FOLDER}/{timestamp_str}_session{session_id}"
        ...
        if relative_times:
            ...
            plt.xlim(min(relative_times) - x_margin if relative_times else -0.5, max(relative_times) + x_margin if relative_times else 0.5)
        if transit_speeds:
    ...
    def _draw_visualizations(self, display_frame, frame_data_obj: FrameData):
        # (Identical to previous version, no changes needed here unless adding new debug info)
    ...
    def _check_timeout_and_reset(self): # General ball detection timeout
        current_system_time = time.monotonic() # Use monotonic for general timeout
        
        # This timeout is for when NO ball has been detected ANYWHERE for detection_timeout_s
        if current_system_time - self.last_detection_timestamp > self.detection_timeout_s:
            if self.is_tracking_r_to_l_transit:
                if self.debug_mode: print(f"DEBUG: R-L transit ENDING by general detection timeout (no ball seen for {self.detection_timeout_s}s).")
                
                # Determine the timestamp of the last valid point in the current transit
                timestamp_of_last_transit_point = current_system_time # Fallback
                if self.use_video_file: timestamp_of_last_transit_point = self.frame_counter / self.actual_fps

                if self.current_r_to_l_transit_data: # Prefer last point in current transit
                    timestamp_of_last_transit_point = self.current_r_to_l_transit_data[-1]['t']
                elif self.trajectory: # Fallback to last trajectory point time
                     timestamp_of_last_transit_point = self.trajectory[-1][2]

                self._finalize_r_to_l_transit(timestamp_of_last_transit_point, "general_detection_timeout")
    ...
    def process_single_frame(self, frame):
        ...
        if motion_mask_roi is not None:
            ...
            if ball_pos_in_roi:
                ...
                self.missed_ball_frames_in_transit = 0 # Reset miss counter on successful detection
                self._calculate_ball_speed()

                cx_global = ball_pos_in_roi[0] + self.roi_start_x
                cy_global = ball_pos_in_roi[1] + self.roi_top_y
                # Use the timestamp from the last point added to trajectory by _detect_ball_in_roi
        ...
        if not ball_detected_this_frame:
            if self.is_tracking_r_to_l_transit: # Only care about missed frames if a transit is active
                self.missed_ball_frames_in_transit += 1
                if self.debug_mode:
                    # print(f"DEBUG: Ball not detected, missed_in_transit: {self.missed_ball_frames_in_transit}")
                    pass
                
                if self.missed_ball_frames_in_transit >= MAX_CONSECUTIVE_MISSES_IN_TRANSIT:
                    if self.debug_mode: print(f"DEBUG: R-L transit ENDING due to {MAX_CONSECUTIVE_MISSES_IN_TRANSIT} consecutive missed frames.")
                    
                    timestamp_of_last_valid_point = time.monotonic() # Fallback
                    if self.use_video_file: timestamp_of_last_valid_point = self.frame_counter / self.actual_fps
                    
                    if self.current_r_to_l_transit_data: # This should contain the last good point
                        timestamp_of_last_valid_point = self.current_r_to_l_transit_data[-1]['t']
                    elif self.trajectory: # Less ideal fallback
                        timestamp_of_last_valid_point = self.trajectory[-1][2]

                    self._finalize_r_to_l_transit(timestamp_of_last_valid_point, "ball_lost_consecutively")
        ...
        if self.debug_mode:
            ...
            pts_count = len(self.current_r_to_l_transit_data)
            miss_count = self.missed_ball_frames_in_transit
            debug_text = f"Traj:{len(self.trajectory)}, Tr:{transit_status}({pts_count}pts,miss:{miss_count})"
        
        frame_data = FrameData(
            frame=frame, roi_sub_frame=roi_sub_frame, 
            ball_position_in_roi=ball_pos_in_roi, ball_contour_in_roi=ball_contour_in_roi,
            current_ball_speed_kmh=self.current_ball_speed_kmh, display_fps=self.display_fps,
    ...
    def run(self):
        # (Identical to previous version, except for window title)
        print("=== Ping Pong Speed Tracker (ROI R-L Transit Avg Speed - v11.2) ===")
        ...
        window_name = 'Ping Pong Speed Tracker v11.2 - ROI Transit Speed'
...
def main():
    ...
    global MIN_BALL_CIRCULARITY, MAX_CONSECUTIVE_MISSES_IN_TRANSIT, MIN_POINTS_FOR_VALID_TRANSIT
    global DEFAULT_DETECTION_TIMEOUT
    
    parser = argparse.ArgumentParser(description='Ping Pong Speed Tracker v11.2 (ROI R-L Avg Speed)')
    ...
    # Expose new robustness parameters
    parser.add_argument('--min_circ', type=float, default=MIN_BALL_CIRCULARITY, help='Minimum ball circularity.')
    parser.add_argument('--max_miss_transit', type=int, default=MAX_CONSECUTIVE_MISSES_IN_TRANSIT, help='Max consecutive missed frames in active transit.')
    parser.add_argument('--min_pts_transit', type=int, default=MIN_POINTS_FOR_VALID_TRANSIT, help='Min data points for a valid transit.')


    parser.add_argument('--debug', action='store_true', default=DEBUG_MODE_DEFAULT, help='Enable debug printouts.')
    args = parser.parse_args()

    # Update global defaults based on args.
    # This is a bit of a workaround because the class constructor uses globals for defaults.
    # A cleaner way would be to pass all these directly to the constructor or have a config object.
    DEFAULT_ROI_START_RATIO = args.roi_start
    DEFAULT_ROI_END_RATIO = args.roi_end
    DEFAULT_ROI_BOTTOM_RATIO = args.roi_bottom
    MIN_BALL_CIRCULARITY = args.min_circ
    MAX_CONSECUTIVE_MISSES_IN_TRANSIT = args.max_miss_transit
    MIN_POINTS_FOR_VALID_TRANSIT = args.min_pts_transit
    DEFAULT_DETECTION_TIMEOUT = args.timeout
    ...
    tracker = PingPongSpeedTracker(
        ...
        detection_timeout_s=args.timeout, # This will be used by the constructor
