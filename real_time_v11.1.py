#!/usr/bin/env python3
# 乒乓球速度追蹤系統 v11 (Refactored for Efficiency and macOS)
...
DEFAULT_DETECTION_TIMEOUT = 0.01
...
# Let OpenCV manage its own threads, or set to a specific number
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM,
        ...
        # self.perspective_ratio = self.far_side_width_cm / self.near_side_width_cm # Not directly used, ratios are calculated per-y
        ...
        for i in range(1, num_labels): # Skip background label 0
            ...
            if MIN_BALL_AREA_PX < area < MAX_BALL_AREA_PX:
                ...
                if max(w_roi, h_roi) > 0:
                    # A simple circularity measure. For more precise, use perimeter from contour.
                    # circularity = min(w_roi, h_roi) / max(w_roi, h_roi)
                    # For a more robust circularity, we might need contours if this isn't good enough.
                    # Let's extract the contour for this component for better circularity.
    ...
    def _calculate_ball_speed(self):
        ...
        if delta_t > 0:
            ...
            # Convert speed to km/h. If t is in seconds, then time_unit is seconds.
            # cm/s to km/h: (cm/s) * (1m/100cm) * (1km/1000m) * (3600s/1h) = val * 0.036
    ...
    def run(self):
        ...
        window_name = 'Ping Pong Speed Tracker v11'
...
def main():
    parser = argparse.ArgumentParser(description='Ping Pong Speed Tracker v11')
    ...
    # Global variable VISUALIZATION_DRAW_INTERVAL can be set via argument if needed
    # global VISUALIZATION_DRAW_INTERVAL
    ...
    # VISUALIZATION_DRAW_INTERVAL = args.vis_interval # If made configurable
