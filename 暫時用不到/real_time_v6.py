...
# 視覺化參數
VISUALIZATION_INTERVAL = 3  # 每隔多少幀繪製一次視覺元素 (軌跡、邊界等)
...
AUTO_STOP_AFTER_COLLECTION = False  # 收集完速度後不自動停止程序
...
class PingPongSpeedTracker:
    ...
    def draw_visualizations(self, frame, roi, ball_position=None, ball_contour=None):
        """
        繪製視覺化元素，根據 VISUALIZATION_INTERVAL 決定繪製頻率
        ...
        """
        # 根據設定的間隔決定是否繪製視覺元素
        if self.frame_count % VISUALIZATION_INTERVAL == 0:
            # 繪製 ROI 邊界
...
def main():
    """主函數：解析命令列參數並啟動追蹤器"""
    global NET_CROSSING_DIRECTION, VISUALIZATION_INTERVAL, MAX_NET_SPEEDS
    ...
    parser.add_argument('--direction', type=str, default=NET_CROSSING_DIRECTION, 
                        ...
                        help='記錄球經過網中心的方向')
    parser.add_argument('--vis_interval', type=int, default=VISUALIZATION_INTERVAL,
                        help='每隔多少幀繪製一次視覺元素（軌跡、邊界等）')
