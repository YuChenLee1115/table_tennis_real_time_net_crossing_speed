#!/usr/bin/env python3
# 乒乓球速度追蹤系統 Windows 優化版 (使用 DirectShow 後端)
# Optimized for Windows with DirectShow backend for high FPS performance
...
# Threading & Queue Parameters for Windows optimization
FRAME_QUEUE_SIZE = 30  # 增加佇列大小以應對 Windows 的潛在性能波動
...
try:
    # 設定 OpenCV 執行緒數目 (Windows 建議限制在實體 CPU 核心數)
...
class FrameReader:
    """Reads frames from camera or video file in a separate thread, optimized for Windows."""
    ...
    def _configure_capture(self, frame_width, frame_height):
        if not self.use_video_file:
            # 設定 MJPG 格式以提高幀率 (Windows DirectShow 支援)
            self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
            
            # 設定緩衝區大小 (Windows 特有參數，增加緩衝減少丟幀)
            ...
            if not self.frame_queue.full():
                ...
                if not ret:
                    if frame_drop_count > 0:
                        print(f"讀取期間總計丟棄 {frame_drop_count} 幀")
                ...
                # 記錄丟幀
                frame_drop_count += 1
                if time.time() - last_drop_report > 5.0:  # 每5秒報告一次
                    print(f"警告：佇列已滿，已丟棄 {frame_drop_count} 幀")
                    last_drop_report = time.time()
                    
                # Windows 優化：避免佇列已滿時過度使用 CPU
                time.sleep(1.0 / (self.target_fps * 8))
...
class PingPongSpeedTracker:
    ...
    def _precalculate_overlay(self):
        ...
        self.instruction_text = "空格: 開始/停止計數 | D: 除錯模式 | Q/ESC: 退出"
    ...
    def _detect_fmo(self):
        ...
        except cv2.error:  # 防呆處理
            _, thresh_mask = cv2.threshold(motion_mask, THRESHOLD_VALUE_FMO, 255, cv2.THRESH_BINARY)
        
        if OPENING_KERNEL_SIZE_FMO[0] > 0:  # 避免核心大小為 (0,0) 的錯誤
    ...
    def _create_output_files(self, net_speeds, relative_times, session_id):
        ...
        # Windows 優化：使用 try-except 防止圖形生成錯誤
        try:
            # Generate Chart
        ...
        # Generate TXT - 使用 Windows 兼容的路徑格式
        try:
    ...
    def _draw_visualizations(self, display_frame, frame_data_obj: FrameData):
        ...
        # 降低視覺化頻率以提高性能，Windows版本可能需要更低的頻率
    ...
    def run(self):
        print("=== Ping Pong Speed Tracker (Windows Optimized) ===")
        ...
        window_name = 'Ping Pong Speed Tracker (Windows Optimized)'
...
def main():
    parser = argparse.ArgumentParser(description='Ping Pong Speed Tracker (Windows Optimized)')
