...
# CENTER_LINE_COLOR_BGR = (0, 255, 255) # Kept for visual reference if desired
...
# EVENT_BUFFER_SIZE_CENTER_CROSS = 200 # No longer needed
# PREDICTION_LOOKAHEAD_FRAMES = 10 # No longer needed
...
class PingPongSpeedTracker:
    def __init__(self, video_source=DEFAULT_CAMERA_INDEX, table_length_cm=DEFAULT_TABLE_LENGTH_CM,
        ...
        self.center_x_global = self.frame_width // 2 # For visual center line if drawn

        self.max_transit_speeds_to_collect = max_transit_speeds
        self.collected_transit_speeds = []
        self.collected_relative_times = []
        self.last_recorded_transit_speed_kmh = 0 # Average speed of last recorded transit
        ...
        self.first_ball_event_timestamp_in_session = None # Timestamp of the start of the first recorded transit
        
        # R-L Transit Tracking State
        self.is_tracking_r_to_l_transit = False
        self.current_r_to_l_transit_data = [] # List of dicts: {'x': global_x, 'y': global_y, 't': timestamp}
    ...
    def _precalculate_overlay(self):
        ...
        # Visual center line (optional, not used for logic)
        # cv2.line(self.static_overlay, (self.center_x_global, 0), (self.center_x_global, self.frame_height), (0, 255, 255), 1) # Thinner visual line
    ...
    def _get_pixel_to_cm_ratio(self, y_global):
        ...
        roi_width_px_for_ratio_calc = self.roi_width_px # Use actual ROI width for this
        if current_width_cm > 0 and roi_width_px_for_ratio_calc > 0:
            pixel_to_cm_ratio = current_width_cm / roi_width_px_for_ratio_calc
        else:
            # Fallback: use nominal table length if ROI width is not well-defined or current_width_cm is zero
            # This fallback is less accurate as it assumes table_length_cm corresponds to frame_width.
    ...
    def _detect_ball_in_roi(self, motion_mask_roi):
        ...
        self.last_detection_timestamp = time.monotonic() # System time for general timeout
        self.trajectory.append((cx_global, cy_global, current_timestamp)) # Crucial: timestamp here is used later
    ...
    def toggle_counting(self):
        ...
        if self.is_counting_active:
            ...
            self.output_generated_for_session = False
            # Reset any ongoing transit if counting is restarted
            self.is_tracking_r_to_l_transit = False
    ...
    def _update_r_to_l_transit_tracking(self, cx_global, cy_global, current_timestamp, current_speed_kmh):
        if not self.is_counting_active:
            if self.is_tracking_r_to_l_transit: # Counting stopped mid-transit
                self._finalize_r_to_l_transit(current_timestamp, "counting_stopped_unexpectedly")
            return

        if len(self.trajectory) < 2: # Not enough data for direction
            if self.is_tracking_r_to_l_transit: # Ball lost reliable trajectory during transit
                self._finalize_r_to_l_transit(current_timestamp, "ball_trajectory_lost")
            return

        prev_x_global, _, _ = self.trajectory[-2] # Second to last point in global trajectory
        
        # Check if ball is within ROI (it should be, as cx_global comes from a ball detected in ROI)
        # but good for sanity if logic changes:
        # if not (self.roi_start_x <= cx_global <= self.roi_end_x and self.roi_top_y <= cy_global <= self.roi_bottom_y):
        #     if self.is_tracking_r_to_l_transit:
        #         self._finalize_r_to_l_transit(current_timestamp, "ball_exited_roi_unexpectedly")
        #     return

        is_moving_left = (cx_global < prev_x_global - 1) # Added small threshold to avoid jitter
        is_moving_right = (cx_global > prev_x_global + 1)
        ...
        if is_moving_left:
            ...
            # Check for exit on the left side of ROI (e.g., within 5% of ROI width from left edge)
            # ROI x-coordinates are global frame coordinates
        ...
        else: # Stalled or vertical movement
            if self.is_tracking_r_to_l_transit:
    ...
    def _finalize_r_to_l_transit(self, end_timestamp, reason):
        if not self.is_tracking_r_to_l_transit or not self.current_r_to_l_transit_data or len(self.current_r_to_l_transit_data) < 2:
            if self.debug_mode and self.is_tracking_r_to_l_transit: # Only print if it was supposed to be a transit
        ...
        end_point_data = self.current_r_to_l_transit_data[-1] # Use actual last point of transit
        ...
        if not self.is_counting_active: # If counting was turned off just before finalize
            if self.debug_mode: print("DEBUG: Counting is OFF during finalize. Speed not recorded.")
            return

        if avg_speed_kmh > 0.1: # Only record meaningful speeds
            if len(self.collected_transit_speeds) >= self.max_transit_speeds_to_collect:
                if not self.output_generated_for_session: # Check to prevent multiple prints/generations
    ...
    def _calculate_ball_speed(self): # Calculates INSTANTANEOUS speed
        if len(self.trajectory) < 2:
        ...
        delta_t = t2 - t1
        if delta_t > 0.0001: # Avoid division by zero for very fast frames
            ...
            if self.debug_mode and speed_kmh > 1: # Print only significant speeds
                 pass # print(f"Instant Speed: {dist_cm:.2f}cm in {delta_t:.4f}s -> Raw {speed_kmh:.1f}km/h, Smooth {self.current_ball_speed_kmh:.1f}km/h")
        else: self.current_ball_speed_kmh *= (1 - SPEED_SMOOTHING_FACTOR)


    def _calculate_real_distance_cm_global(self, x1_g, y1_g, x2_g, y2_g):
        y1_roi = max(0, min(self.roi_height_px, y1_g - self.roi_top_y)) # Clamp y to be within ROI height for lookup
        ...
        avg_cm_per_pixel_ratio = (ratio1 + ratio2) / 2.0 # This is cm/pixel from _get_pixel_to_cm_ratio
    ...
    def _create_output_files(self, transit_speeds, relative_times, session_id):
        ...
        if transit_speeds:
            ...
            plt.ylim(max(0, min_speed - y_range*0.1), max_speed + y_range*0.1) # Ensure y starts at 0 or just below min_speed
    ...
    def _draw_visualizations(self, display_frame, frame_data_obj: FrameData):
        ...
        if frame_data_obj.ball_position_in_roi and frame_data_obj.roi_sub_frame is not None:
            ...
            # ROI sub_frame is a view, so modifications are reflected in vis_frame (display_frame)
            # cx_global, cy_global = cx_roi + self.roi_start_x, cy_roi + self.roi_top_y
            # cv2.circle(vis_frame, (cx_global, cy_global), 8, BALL_COLOR_BGR, -1) # Draw on main frame too
    ...
    def _check_timeout_and_reset(self): # General ball detection timeout
        current_time = time.monotonic() # System time for timeout check
        if current_time - self.last_detection_timestamp > self.detection_timeout_s:
            if self.is_tracking_r_to_l_transit:
                ...
                last_point_event_time = current_time # Default if no data
                if self.use_video_file: # For video, use frame-based time
                    last_point_event_time = self.frame_counter / self.actual_fps
                
                if self.current_r_to_l_transit_data: # If transit data exists, use its last timestamp
                    last_point_event_time = self.current_r_to_l_transit_data[-1]['t']
                elif self.trajectory: # Fallback to trajectory's last known time
                     last_point_event_time = self.trajectory[-1][2]

                self._finalize_r_to_l_transit(last_point_event_time, "detection_timeout")
            
            self.trajectory.clear()
            self.current_ball_speed_kmh = 0 # Reset instantaneous speed
    ...
    def process_single_frame(self, frame):
        ...
        if motion_mask_roi is not None:
            ball_pos_in_roi, ball_contour_in_roi = self._detect_ball_in_roi(motion_mask_roi) # Updates self.trajectory and self.last_detection_timestamp
            
            if ball_pos_in_roi:
                ball_detected_this_frame = True
                self._calculate_ball_speed() # Calculates self.current_ball_speed_kmh (instantaneous)
                ...
                # This ensures consistency between trajectory and transit data timestamps
                ...
                # --- End R-L Transit Logic ---
        
        if not ball_detected_this_frame: # Ball NOT detected by FMO or selection
            if self.is_tracking_r_to_l_transit:
                if self.debug_mode: print(f"DEBUG: Ball not detected in frame, potentially ending R-L transit.")
                
                last_known_event_time = time.monotonic() # Default for live
                if self.use_video_file:
                ...
                self._finalize_r_to_l_transit(last_known_event_time, "ball_not_detected_in_frame")
        ...
        if self.debug_mode:
            ...
            debug_text = f"Traj: {len(self.trajectory)}, Transit: {transit_status} ({pts_count}pts)"
    ...
    def run(self):
        ...
        window_name = 'Ping Pong Speed Tracker v11.1 - ROI Transit Speed'
            ...
            while self.running:
                ...
                if not ret or frame is None:
                    ...
                    if self.is_counting_active and self.collected_transit_speeds and not self.output_generated_for_session:
                        ...
                        self._generate_outputs_async() # Ensure this uses the correct lists
                        self.output_generated_for_session = True
                    break
...
def main():
    ...
    # Override default ROI ratios if provided
