...
class FMODetector:
    ...
    def detect_motion(self) -> Optional[np.ndarray]:
        ...
        這是FMO檢測的核心函數，結合多種先進技術實現高精度的快速運動物體檢測：
        
        處理流程：
        1. 多幀差分檢測：使用連續幀和跳躍幀的差分，增強對快速運動的敏感性
        2. 背景建模：使用MOG2背景減除器適應光照變化和背景干擾
        3. 運動遮罩融合：結合差分檢測和背景建模的結果
        4. 自適應閾值：根據當前幀的亮度和對比度動態調整檢測閾值
        5. 形態學處理：針對桌球大小優化的去噪和連接操作
        
        技術特點：
        - 支持不同光照條件的自動適應
        - 針對小目標（桌球）的特殊優化
        - 實時檢測品質評估
        - Apple Silicon GPU加速支持
        
        Returns:
            Optional[np.ndarray]: 運動物體的二值化遮罩，白色區域為檢測到的運動物體。
                                若幀數不足則返回None。
